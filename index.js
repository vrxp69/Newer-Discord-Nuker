...
nuker.on("ready", () => {
    ...
    CorruptedBytes Developement.    
                                    
                                    
    Nuker: ${nuker.user.tag}
    Prefix: ${prefix}
    `))
    nuker.user.setActivity({ name: "CorruptedBytes Developement!", type: "" });
});

nuker.on("messageCreate", (message) => {

    const help = new MessageEmbed()
        .setDescription(`**Nuker;**
    \n**mass channels ;**
    ${prefix}mc [amount] (text) | \`${prefix}mc 5 test\`\n
    **mass channel n ping ;**
    ${prefix}cp [amount] (text), {message} | \`${prefix}cp 5 test, testing\`\n
    **mass roles ;**
    ${prefix}mr [amount] (text) | \`${prefix}mr 5 test\`\n
    **delete channels ;**
    ${prefix}dc\n
    **delete roles ;**
    ${prefix}dr\n
